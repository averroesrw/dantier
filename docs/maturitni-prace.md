...
Úvod a cíl práce
Způsob objednávání služeb se v posledních letech výrazně proměnil. Zákazníci stále častěji upřednostňují online rezervace před klasickým telefonátem nebo osobní návštěvou, především díky jejich rychlosti a nepřetržité dostupnosti. Tento posun se přirozeně dotýká i kadeřnických salonů, které na poptávku reagují zaváděním moderních rezervačních systémů.
...
Teoretická část
Tato kapitola představuje klíčové technologie a koncepty využité při tvorbě aplikace. Znalost těchto teoretických principů tvoří základ pro pochopení postupů, které jsou následně popsány v praktické části.
...
Pro použití TypeScriptu ve Vue komponentách stačí jednoduše přidat atribut lang="ts" do tagu <script>. Samotný framework Vue od své třetí verze TypeScript plně podporuje, protože v něm byl přímo napsán. Díky tomu jsou automaticky k dispozici hotové typové definice pro všechna vestavěná API..
